...
if numerators[0] < 0:
    if numerators[1] < 0:
        if numerators[2] < 0:
            displayProblem = "\\frac{%d x - %d}{%s} - \\frac{%d x - %d}{%d} = \\frac{%d x - %d}{%d}" %(coefficients[0], -numerators[0], denominators[0], coefficients[1], -numerators[1], denominators[1], coefficients[2], -numerators[2], denominators[2])
        else:
            displayProblem = "\\frac{%d x - %d}{%s} - \\frac{%d x - %d}{%d} = \\frac{%d x + %d}{%d}" %(coefficients[0], -numerators[0], denominators[0], coefficients[1], -numerators[1], denominators[1], coefficients[2], numerators[2], denominators[2])
    else:
        if numerators[2] < 0:
