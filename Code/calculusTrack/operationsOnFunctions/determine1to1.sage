...
print "Starting Module 10 Objective 3"
...
def generate1ZeroOddDegree():
    ...
    function = term**3
    if a < 0 and b < 0:
        displayFunction = "%d x^3 - %d x^2 - %d x - %d" %(a**3, -3*a**2*b, -3*a*b**2, -b**3)
    elif a < 0 and b > 0:
        displayFunction = "%d x^3 + %d x^2 - %d x + %d" %(a**3, 3*a**2*b, -3*a*b**2, b**3)
    elif a > 0 and b < 0:
        displayFunction = "%d x^3 - %d x^2 + %d x + %d" %(a**3, -3*a**2*b, 3*a*b**2, b**3)
...
generalComments = "General Comments: There are only two valid options: The function is 1-1 OR No because there is a $y$-value that goes to 2 different $x$-values."
...
print "I have completed Module 10 Objective 3, Master"
